...
router.get('/profiles', middlewere.MustBeLoggedIn, middlewere.MustHaveProfile, (req, res) => {
  const allProfiles = [];
  const url = `http://api.geonames.org/findNearbyPostalCodesJSON?postalcode=${req.user.profile.postalcode}&country=${req.user.profile.countrycode}&maxRows=10&radius=30&username=balend`;
  request(url, (error, response, body) => {
    if (!error && response.statusCode === 200) {
      const parsedData = JSON.parse(body);
      for (let i = 0; i < parsedData.postalCodes.length; i += 1) {
        allProfiles.push(parsedData.postalCodes[i].postalCodes);
      }
      Profile.paginate({}, { page: 1, sort: { registered: 1 }, limit: 4 }, (error, everyProfiles) => {
        if (error) {
          req.flash('error', `${error.message}`);
          res.render('profiles/index');
        }
        Profile.paginate({ postalcode: { $in: allProfiles } }, { sort: { registered: 1 }, limit: 4 }, (error, nearbyProfiles) => {
...
router.post('/profiles', middlewere.MustBeLoggedIn, upload.single('profileImage'), middlewere.ValidateProfileLocation, middlewere.ValidateProfileAge, (req, res) => {
  ...
  Profile.create(newProfile, (error, newAddedProfile) => {
    if (error) {
      req.flash('error', `${error.message}`);
      res.redirect('profiles/new');
    } else {
      const UsersProfile = {
        id: newAddedProfile._id,
        firstname: newAddedProfile.firstname,
        postalcode: newAddedProfile.postalcode,
        countrycode: newAddedProfile.countrycode,
      };
      User.findByIdAndUpdate(req.user._id, { profile: UsersProfile }, (error) => {
        if (error) {
          req.flash('error', `${error.message}`);
          res.redirect('profiles/new');
        }
...
router.put('/profiles/:id', middlewere.MustBeLoggedIn, middlewere.MustHaveProfile, upload.single('profileImage'), (req, res) => {
  ...
  Profile.findByIdAndUpdate(req.params.id, updatedProfile, (error) => {
    if (error) {
      req.flash('error', `${error.message}`);
